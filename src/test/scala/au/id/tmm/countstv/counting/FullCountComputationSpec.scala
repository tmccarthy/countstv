...
class FullCountComputationSpec extends ImprovedFlatSpec {
  ...
  private val papers = Vector[Vector[Fruit]](
  ...
  private val testPreferenceTree = PreferenceTree.from[Fruit](papers)
  ...
  it should "produce the correct outcome when there is a tie" in {
    val newPapers: Vector[Vector[Fruit]] =
      papers.tail :+ Vector(Watermelon, Apple, Raspberry, Mango, Banana, Pear, Strawberry)

    val preferenceTree = PreferenceTree.from[Fruit](newPapers)
