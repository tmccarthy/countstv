...
object BallotNormalisation {
  ...
  object Result {
    ...
    final case class Formal[C](normalisedBallot: ArraySeq[C]) extends Result[C]

    final case class Saved[C](normalisedBallot: ArraySeq[C], rulesViolated: Set[BallotNormalisationRule])
        extends Result[C]
  ...
  def normalise[C](
    ...
    def orderAccordingToPreferences(preferences: Map[C, Preference]): ArraySeq[Set[C]] = {
      val returnedArraySeq: scala.collection.mutable.Buffer[Set[C]] =
        ArraySeq.fill(preferences.size)(Set.empty[C]).toBuffer
    ...
    val mandatoryRulesViolated = (potentialViolations intersect mandatoryRules.rules) ++ mandatoryRules.activeMinimumPreferencesRule
      .filter { case MinimumPreferences(numPreferences) => formalPreferences.size < numPreferences }
    val optionalRulesViolated = (potentialViolations intersect optionalRules.rules) ++ optionalRules.activeMinimumPreferencesRule
      .filter { case MinimumPreferences(numPreferences) => formalPreferences.size < numPreferences }
