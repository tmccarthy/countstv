...
object AllocationAfterIneligiblesComputation {
  ...
  def distributeAwayFromIneligibles[C](
    ...
    val newPaperBundlesPerIneligibleCandidate = oldContext.paperBundles
      ...
      .map {
        case (ineligibleCandidate, paperBundlesForIneligibleCandidate) =>
    ...
    val transfersDueToIneligibles = newPaperBundlesPerIneligibleCandidate.map {
      case (candidate, paperBundles) =>
    ...
    NextActionComputation
      .computeNextAction(oldContext.numVacancies, oldContext.quota, proposedCountSteps)
      .map {
        case NewStatusesAndNextAction(newCandidateStatuses, nextAction) =>
