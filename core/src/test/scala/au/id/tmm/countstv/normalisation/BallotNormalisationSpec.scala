...
class BallotNormalisationSpec extends FlatSpec {
  ...
  private def UnNormalisedBallot(preferences: (Fruit, Any)*): UnNormalisedBallot[Fruit] =
    preferences.toMap.view.mapValues {
  ...
  private def testRule(
    ...
    passingBallots.foreach {
      case (ballotDescription, (unNormalisedBallot, expectedNormalisedBallot)) =>
    ...
    violatingBallots.foreach {
      case (ballotDescription, (unNormalisedBallot, expectedNormalisedBallot)) =>
        it should s"warn for $ballotDescription when it is an optional rule" in {
          val normalisedBallot =
            BallotNormalisation.normalise(BallotNormalisationRules.laxest, rules)(unNormalisedBallot)
        ...
        it should s"fail for $ballotDescription when it is an optional rule" in {
          val normalisedBallot =
            BallotNormalisation.normalise(rules, BallotNormalisationRules.laxest)(unNormalisedBallot)
  ...
  testRule(
    ...
    passingBallots = List(
      "a ballot with 5 preferences" -> (
        UnNormalisedBallot(Apple -> 1, Banana -> 2, Mango -> 3, Peach -> 4, Pear -> 5) -> ArraySeq(
          Apple,
          Banana,
          Mango,
          Peach,
      ...
      "a ballot that misses a number after the third preference" -> (
        UnNormalisedBallot(Apple -> 1, Banana -> 2, Mango -> 3, Peach -> 4, Pear -> 6) -> ArraySeq(
          Apple,
          Banana,
          Mango,
          Peach),
      ...
      "a ballot that repeats a number after the third preference" -> (
        UnNormalisedBallot(Apple -> 1, Banana -> 2, Mango -> 3, Peach -> 4, Pear -> 4, Raspberry -> 5) -> ArraySeq(
          Apple,
          Banana,
          Mango),
      ),
